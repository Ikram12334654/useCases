...
Exposes `render()`, called by the hub page wrapper in ``pages/``. Page config
(title/layout) is set by the wrapper before this runs.
    ...
    with right:
        ...
        if not order or not match:
            ...
            if st.session_state.created is None:
                ...
                with a1:
                    approve = st.button(
                        "✅ Approve Order", type="primary", use_container_width=True
                    )
                with a2:
